      ...
      connection.query('SELECT * FROM cs_students where student_active_email=?',req.body['student_active_email'],
        function(err, rows) {
            if (err) {
              ...
              res.status(303).json({'message': 'Student already present in current certificate programme','code': 'Student Exists'});
            }else{
              connection.query("INSERT INTO `cs_students` SET ?",req.body,function (err, results) {
                 if (err) {
                  ...
                  res.status(200).json({'message': 'Student created Successfully' ,'code': 'SUCCESS'});
                 }
              });
            }
        });
  ...
  connection.query("SELECT * FROM cs_students where id_added=?" , [req.query.member_id] ,
      function(err, studata) {
          if(studata && studata.length>0){
            res.status(200).json({'student' : studata });
          }else{
            res.status(401).json({'message': 'Oops! Something went wrong!!' ,'code': 'Invalid Details'});
          }
      })
      ...
      if(results.affectedRows)
        {
          ...
          res.status(200).json({'message': 'Student updated Successfully' ,'code': 'SUCCESS'});
        }
     }
  });
});
  ...
  connection.query("Delete from cs_students where student_id=?" , [req.query.student_id] ,
      function(err, results) {
          if(err){
            ...
            res.status(200).json({'message': 'Student deleted Successfully' ,'code': 'SUCCESS'});
